...
def jobs_categories_getdata(request):
    if request.method == 'POST':
        ...
        sql = "select total.name, coalesce(total.total_jobs,0), coalesce(total.paid_invoices,0), coalesce(total.paid_amount,0), round(100 * total_jobs/ sum(total_jobs) over(), 2), round(100 * total.converted_jobs::numeric/ total_jobs::numeric, 2) from (select sc.name, count(distinct cj.id) as total_jobs, count(distinct case when ci.status=4 then ci.message_ptr_id else null end) as paid_invoices, count(distinct case when ci.status=4 then cj.id else null end) as converted_jobs, sum(distinct case when ci.status=4 then ci.amount else null end) as paid_amount  from skills_category sc inner join contracts_job cj on cj.category_id=sc.id  left outer join contracts_application ca on ca.job_id=cj.id left outer join contracts_message cm on cm.application_id= ca.id left outer join contracts_invoice ci on ci.message_ptr_id=cm.id  where cj.approved=true and cj.created_at >= '"+fromdate+"' and cj.created_at<= '"+todate+"' group by sc.id ) total order by total_jobs desc "
...
def dashboard_getdata(request):
    if request.method == 'POST':
        ...
        proposalspaid_sql = ("(select "+datefieldtostring("cp.payed_timestamp", grouppertext) + " as proposalpaid,sum(case when cp.status=4 then 1 else 0 end) as paidproposal_count from contracts_proposal cp inner join contracts_message cm on cp.message_ptr_id=cm.id inner join contracts_application ca on ca.id=cm.application_id inner join contracts_job cj on cj.id=ca.job_id inner join users u on u.id=cj.employer_id where cp.payed_timestamp>='"+t1+"' and cp.payed_timestamp<='"+t2+"' "+languagecodesql('u.site_lang',sitelang)+" group by proposalpaid) paidproposals on paidproposals.proposalpaid=contractsmessages.msgdate left outer join ")  
        ...
        assistedjobs_sql =("(select count(id) as assistedjob_count,  "+datefieldtostring("created_at", grouppertext) + " as createdat from contracts_job  where approved=true and assisted=true and created_at>='"+t1+"' and created_at<='"+t2+"' "+languagecodesql('site_lang',sitelang)+" group by createdat) assistedjobs on assistedjobs.createdat=contractsmessages.msgdate  left outer join")
        
        sql = (header_sql + workflow_messages_sql + allusers_sql + freelancers_sql + employers_sql + realemployers_sql + jobs_sql + contractsmessages_sql + porposals_sql + proposalspaid_sql + application_sql +invited_sql+ invoicesent_sql + invoicepaid_sql + depositrequestsent_sql + depositrequestpaid_sql + invperjob_sql+ assistedjobs_sql +appperjob_sql + "  order by msgdate")
...
def jobs_applications_statistics_getdata(request):
    if request.method == 'POST':
        ...
        searchsql = ""
        if keywords <> "":
            searchsql = "and (lower(substring(cj.title,1,40)) like '%%" +keywords.lower() + "%%' or lower(substring(au.email,1,40)) like '%%" +keywords.lower() + "%%' or lower(substring(u.first_name || ' ' || u.last_name,1,40)) like '%%" +keywords.lower() + "%%')"
        
        contsearchsql = "" 
        if contkeywords <> "":    
            contsearchsql = " and cj.id in (select distinct contcj.id from users cont inner join auth_user contauth on contauth.id=cont.django_user_id inner join contracts_application contapp on contapp.applicant_id=cont.id  inner join contracts_job contcj on contcj.id=contapp.job_id  where (lower(substring(contauth.email,1,40)) like '%%" +contkeywords.lower() + "%%' or  lower(substring(cont.first_name || ' ' || cont.last_name,1,40)) like '%%" +contkeywords.lower() + "%%') ) "
...
def jobs_communications_getdata(request):
    if request.method == 'POST':
        ...
        sql = ("select u.id,au.email,au.first_name || ' ' || au.last_name as freelancer_name, u.countrycode || ' ' || u.areacode || ' ' || u.mobile as phone,ca.id as application_id,case when ca.shortlisted=true then 1 else 0 end as shortlisted,count(cp.message_ptr_id) as proposals, sum(case when cp.status=4 then 1 else 0 end) as acceptedproposal_count, sum(case when cm.from_applicant=false then 1 else 0 end) as employer_responses, ca.client_views from contracts_application ca inner join users u on u.id=ca.applicant_id inner join auth_user au on u.django_user_id=au.id inner join contracts_message cm on cm.application_id=ca.id left outer join contracts_proposal cp on cp.message_ptr_id=cm.id where ca.job_id= "+job_id+" group by u.id,au.email,freelancer_name, phone, ca.id,shortlisted ;")
...
def paymentstracking_getdata(request):
    if request.method == 'POST':
        ...
        sql = ("select cpt.id, au.first_name || ' ' ||  au.last_name as fullname,  fra.first_name || ' ' ||  fra.last_name as frfullname, cpt.amount,cpt.card_number, cpt.card_holder_name, to_date(cpt.transaction_date, 'MM/DD/YY'), cm.public_id, cpt.order_reference, cj.id as job_id, ca.id as workstream_id , case when pendingtransfers.amount>0 then 'Pending ' || pendingtransfers.amount else '' end as status from contracts_payforttransaction cpt inner join contracts_message cm on cm.id=cpt.payable_id inner join contracts_application ca on ca.id=cm.application_id inner join contracts_job cj on cj.id=ca.job_id inner join users u on u.id=cj.employer_id inner join auth_user au on au.id=u.django_user_id inner join users fr on fr.id=ca.applicant_id inner join auth_user fra on fra.id=fr.django_user_id  left outer join (select cm.public_id, cdt.amount from contracts_pendingtransfers cdt inner join  contracts_message cm on cm.id=cdt.invoice_id) pendingtransfers on pendingtransfers.public_id=cm.public_id where card_number<>'pending'  and to_date(cpt.transaction_date, 'MM/DD/YY') >= '"+t1+"' and to_date(cpt.transaction_date, 'MM/DD/YY') <='"+t2+"' order by cpt.id desc ")
...
def getcategoriesbubbles(request):
    if request.method == 'GET':
        #objs = simplejson.loads(request.body)                            
        sql = "select * from (select 'main' as name, null, 1 as size from skills_category union select name, 'main' as name, 2 as size from skills_category union select ss.name, sc.name , count(distinct su.id) * 10 from skills_skill ss inner join skills_users su on su.skill_id=ss.id inner join skills_category sc on sc.id=ss.category_id group by ss.id,sc.id) total order by size" 
        print sql
