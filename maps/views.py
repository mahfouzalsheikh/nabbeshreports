...
def growthdashboard_getdata(request):
    if request.method == 'POST':
        ...
        workflow_messages_sql = ("(select count(distinct id)/ (sum(count(distinct id)) over (order by "+datefieldtostring("timestamp", grouppertext) + ") -count(distinct id) +0.0001) as message_count,"+datefieldtostring("timestamp", grouppertext) + " as msgdate from contracts_message where timestamp>='"+t1+"' and timestamp<='"+t2+"' group by msgdate) contractsmessages left outer join ")
